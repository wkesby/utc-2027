...
4. **Actions → UTC standings → Run workflow** once to prove it. The first run creates `docs/standings.json` and the fixtures. A plain manual run is always a quiet refresh — tick the *report* box to also build and send the weekly report (that's what emails and pings phones, so leave it unticked unless you mean it). The *resend* box emails the newest wrap again exactly as it stands — no rebuild, no phone push — for when the email went astray.
...
- Any time a drafted team loses to a team nobody drafted, "The Commentator" posts a sledge to the banter wall aimed at the owner — written by Claude when `ANTHROPIC_API_KEY` is set, canned otherwise — and it goes out as a push like any other wall post. One sledge per defeat, never repeated. Every such defeat also earns one **demerit** on the ledger — a shame table on its own page in the app, entirely separate from the real scoring. With the instant upgrade below deployed, a *sledge patrol* in the Cloud Function checks ESPN's scoreboards every 10 minutes and books the sledge and demerit within minutes of full time; the hourly workflow then just mirrors the offences onto the committed ledger (`docs/demerits.json`) and acts as the safety net.
...
- 14:00 Melbourne Tuesday (or the first scheduled run after it — GitHub runs schedules late): report built, ladder image rendered, **pushed as a notification to every phone that opted in** (tap it to read the wrap in the app — it also sits at the top of the Ladder tab), and emailed to you with a **one-tap WhatsApp share link** for anyone still on the old channel — tap, choose the group, send. WhatsApp has no official way for software to post into a group, so that tap stays human; notifications from the app itself don't need it.
